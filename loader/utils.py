...
def get_events(event_path, type):
    # It's possible that there is no event file! (camera standing still)
    try:
        if type == 'test':
            ## original mvsec outdoor_day1 (.h5 )
            #f = pandas.read_hdf(event_path, "myDataset")
            ## indoor_flying1 
            event_npy = numpy.load(event_path)
            col_names = ['x', 'y', 'ts', 'p']
            f = pandas.DataFrame(event_npy, columns = col_names)
          
        elif (type == 'train') | (type == 'valid'):
            event_npy = numpy.load(event_path)
            col_names = ['x', 'y', 'ts', 'p']
            f = pandas.DataFrame(event_npy, columns = col_names)
...
def get_flow_npy(flow_path, type):
    # Array 2,height, width
    # No scaling needed.
    if type == 'test':
        ## 정인언니 데이터셋만 
        flow = numpy.load(flow_path, allow_pickle=True)
        # Only in 정인 언니 데이터셋 
        #flow = numpy.transpose(flow, (2, 0, 1))
        return flow  
        # return numpy.load(flow_path, allow_pickle=True)
    elif (type == 'train') | (type == 'valid'):
        flow = numpy.load(flow_path, allow_pickle=True)
        # transpose -> ( 정인언니 데이터셋에서만 진행 )
        #flow = numpy.transpose(flow, (2, 0, 1))
        return flow    
