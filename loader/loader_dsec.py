...
class Sequence(Dataset):
    def __init__(self, seq_path: Path, representation_type: RepresentationType, mode: str='test', delta_t_ms: int=100,
        ...
        if mode == 'test':
            ...
            train_timestamp_file = seq_path / 'train_forward_flow_timestamps.csv'
            assert train_timestamp_file.is_file()
            file = np.genfromtxt(
        ...
        self.flow_file = seq_path / 'optical_flow'
    ...
    def get_data_sample(self, index, crop_window=None, flip=None):
        ...
        if self.mode == 'test':
            ...
            ## load Optical Flow
            flow_file_name = os.listdir(self.flow_file)
            for i in flow_file_name:
                num_flow_file_name = int(i[:-4])
                if file_index == num_flow_file_name:
                    flow_path = self.flow_file / i
                    flows, valid2d = self.load_flow(flow_path)
                    flows = flows.transpose(2, 0, 1) # (480, 640, 2) -> ( 2, 480, 640 ) _ array 
                    valid2d = np.stack((valid2d, valid2d), axis = 0)
                    break
                else:
                    continue
            for i in range(len(names)):
                ...
                ## Random 변수 설정 ( 첫번째만 설정해주고, new에서 동일 랜덤 변수 사용해야함 )
                if i == 0:
                    # Random Cropping 
                    crop_size_height = 288
                    crop_size_width = 384 
                    # random starting point ( y, x )
                    y0 = np.random.randint(0, event_representation.shape[1] - crop_size_height)
                    x0 = np.random.randint(0, event_representation.shape[2] - crop_size_width)
                    # Random Flipping 변수 
                    rand_h = np.random.rand()
                    rand_v = np.random.rand()
                
                event_representation = event_representation.detach().cpu().numpy()
                event_representation = event_representation[:,y0:y0+crop_size_height, x0:x0 + crop_size_width]
                if i == 0:
                    flows = flows[:,y0:y0+crop_size_height, x0:x0 + crop_size_width]
                    valid2d = valid2d[:, y0:y0+crop_size_height, x0:x0 + crop_size_width]
                ...
                if rand_h < h_flip_prob:
                    # event voxel 
                    event_representation = event_representation[:, :, ::-1]
                    ...
                    if i == 0:
                        ...
                        flows[0] = flows[0] * -1.0
                        flows[1] = flows[1] * 1.0
                        #valid2d = valid2d.detach().cpu().numpy()
                        valid2d = valid2d[:, :, ::-1]
                ...
                # Event voxel save to "output"
                event_representation = torch.from_numpy(event_representation.copy())
                output[names[i]] = event_representation
                output['name_map'] = self.name_idx
            
        output['flow'] = torch.from_numpy(flows.copy())
        if str(type(valid2d)) == "<class 'numpy.ndarray'>":
            output['gt_valid_mask'] = torch.from_numpy(valid2d.copy())
        elif str(type(valid2d)) == "<class 'torch.Tensor'>":
            output['gt_valid_mask'] = valid2d
        else:
            assert (type(valid2d) == "<class 'numpy.ndarray'>") or (type(valid2d) == "<class 'torch.Tensor'>"), "Flow GT Mask is wrong!"
...
class DatasetProvider:
    ...
    def __init__(self, dataset_path, representation_type, delta_t_ms, num_bins, type, config, visualize, mode, transforms):
        #test_path = dataset_path / 'test'
        if mode == 'test':
            mode_path = dataset_path
            test_sequences = list()
        ...
        assert mode_path.is_dir(), str(mode_path)
        assert delta_t_ms == 100
        self.config=config

        ## child : /share/data/train/thun_00_a 와 같은 경로가 들어감
        for child in mode_path.iterdir():
            if mode == 'test':
                self.name_mapper_test.append(str(child).split("/")[-1])
            elif mode == 'train':
                # thun_00_a 폴더 명이 append 됨 
            ...
            if type == 'standard':
                ...
                if mode == 'test':
                    test_sequences.append(SequenceRecurrent(child, representation_type, 'test', delta_t_ms, num_bins,
                                               transforms=transforms,
                                               sequence_length=config['data_loader']['train']['args']['sequence_length'],
                                               name_idx=len(self.name_mapper_test)-1,
                                               visualize=visualize))
                elif mode == 'train':
