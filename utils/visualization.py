...
class BaseVisualizer(object):
    def __init__(self, dataloader, save_path, additional_args=None):
        ...
        if os.path.isdir(self.submission_path) == True:
            pass
        else:
...
class FlowVisualizerEvents(BaseVisualizer):
    ...
    def visualize_events(self, image, batch, batch_idx, epoch=None, flip_before_crop=True, crop_window=None):
        ...
        # Load Raw events
        events = self.dataloader.dataset.get_events( loader_idx=int(batch['loader_idx'][batch_idx].item()), train_val_test_type=self.mode )
...
def visualize_optical_flow(flow, savepath=None, return_image=False, text=None, scaling=None):
    ...
    if text is not None:
        plt.text(0, -5, text)
    # savepath : flow_gt, flow, masked
    if savepath is not None:
        if plot_with_pyplot:
            fig.savefig(savepath, bbox_inches='tight', dpi=200)
            plt.close()
        else: #Plot with skimage
            # bgr : ( 256, 256, 3 ) _ min : 0, max : 1
            # out : ( 256, 256, 3 ) _ min : 0, max : 255
            out = bgr*255
            # uint8 : 양수, 2^8 (0~255) 까지 표현가능한 data type
